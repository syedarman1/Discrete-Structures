...
# Label them f0 through f15, to match the order in Wikipedia. All sixteen should have two propositional variables p and q, even if the function uses only one or the other. Verify that the output matches what is expected
def f0_false(p,q):
    return False
...
# #[10] Define a function is_equivalent(f1, f2)
# that determines if two functions are logically equivalent by building the truth tables for
# both and comparing outputs, or by evaluating f1 ↔ f2 to be a tautology.
...
def is_equivalent(f1, f2):
    ...
    return result1[0] == result2[0] and result1[1] == result1[1] and all([result1[4][i] == result2[4][i] for i in range(result1[0])])
